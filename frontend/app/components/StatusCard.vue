  ...
  <div class="w-full max-w-lg bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
    ...
    <div class="p-8 pb-0 flex justify-between items-start">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">MySql + Reddis Handshake</p>
      </div>
      ...
      <div class="flex items-center gap-2 px-3 py-1 rounded-full border transition-all duration-500"
        ...
        <span class="relative flex h-2 w-2 text-center">
          <svg v-if="!data || pending" class="animate-spin h-2 w-2 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div class="p-8">
      ...
      <div v-if="error" class="p-4 bg-red-50 border border-red-100 rounded-2xl">
      ...
      <div v-else class="space-y-4">
        <div :class="[
          ...
          <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">MySQL Version</p>
            <p class="text-gray-800 font-semibold text-center text-sm">
              {{ data && !pending ? data.database_version : '&nbsp;' }}
            </p>
          </div>
          ...
          <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            <p :class="[
                ...
                data?.redis_status === 'Online' ? 'text-green-700' : 'text-red-700'
              ]"
              class="font-bold text-sm"
            >
              {{ data && !pending ? data.redis_status : '&nbsp;' }}
            </p>
          </div>
          ...
          <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            <p class="text-gray-800 font-semibold text-center text-sm">
              {{ data && !pending ? `v${data.php_version}` : '&nbsp;' }}
            </p>
          </div>
    ...
    <div class="p-8 bg-gray-50/50 border-t border-gray-100 flex justify-center">
      ...
      >
        <svg v-if="!data ||pending" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        {{ !data || pending ? 'Updating...' : 'Refresh Status' }}
      </button>
    </div>
  </div>
</template>